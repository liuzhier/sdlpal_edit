								...
								BOOL bSelectPastedCells /*=TRUE*/)
{
    ...
    do
    {
        ...
        CString strNext = (nIndex < 0)? CString( _T(""))  : strLine.Mid(nIndex + 1);
        ...
        while (!strLine.IsEmpty())
        {
            ...
            strLine = (nLineIndex >= 0)? strLine.Mid(nLineIndex + 1) : CString( _T(""));
    ...
    TRY
    {
        ...
        for (int i = 0; i < GetRowCount(); i++)
        {
            for (int j = 0; j < nNumColumns; j++)
            {
                ...
                File.WriteString((j==(nNumColumns-1))?CString( _T("\n")): strSeparator);
            }
        }
